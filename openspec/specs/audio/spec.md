...
`SoundManager` (in `audio.py`) generates all chiptune sounds programmatically at construction time using Python's stdlib `array` module and `pygame.mixer.Sound`. No external audio files are loaded. When `config.sound_enabled` is `False`, or when `pygame.mixer` fails to initialise, every method is a no-op. `audio.py` is one of two modules that import pygame (the other is `renderer.py`).
...
### Requirement: SoundManager.close() stops all managed channels
`SoundManager.close()` stops any active music channel. It is called in the `finally` block of `cli.main()` alongside `renderer.close()`.
